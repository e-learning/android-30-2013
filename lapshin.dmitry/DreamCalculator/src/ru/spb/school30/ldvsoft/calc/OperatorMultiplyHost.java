 ...
 * Date: 04.11.13
 * Time: 20:40
 * MathExpression for operator * and it's Host
 */
public class OperatorMultiplyHost implements FunctionHost
{
	@Override
	...
	public MathExpression buildFunction(Vector<MathExpression> params) throws SyntaxErrorException
	{
		...
		if (Calculator.getMode() == Calculator.Mode.REAL)
			return new NumberMultiplyExpression(op1, op2);
		throw new SyntaxErrorException("No known operator * for these params.");
	}
}

class NumberMultiplyExpression implements MathExpression
{
	MathExpression left, right;

	public NumberMultiplyExpression(MathExpression aLeft, MathExpression aRight)
	{
		left = aLeft;
		right = aRight;
	}
