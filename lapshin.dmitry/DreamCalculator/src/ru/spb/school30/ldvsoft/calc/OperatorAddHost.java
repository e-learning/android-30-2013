 ...
 * Date: 04.11.13
 * Time: 20:40
 * MathExpression for operator + with Host
 */
public class OperatorAddHost implements FunctionHost
{
	@Override
	...
	public MathExpression buildFunction(Vector<MathExpression> params) throws SyntaxErrorException
	{
		...
		if (Calculator.getMode() == Calculator.Mode.REAL)
			return new NumberAddExpression(op1, op2);
		throw new SyntaxErrorException("No known operator + for these params.");
	}
}

class NumberAddExpression implements MathExpression
{
	MathExpression left, right;

	public NumberAddExpression(MathExpression aLeft, MathExpression aRight)
	{
		left = aLeft;
		right = aRight;
	}
