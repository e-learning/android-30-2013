 ...
 * Date: 04.11.13
 * Time: 20:40
 * MathExpression for operator - with it's Host
 */
public class OperatorSubstractHost implements FunctionHost
{
	@Override
	...
	public MathExpression buildFunction(Vector<MathExpression> params) throws SyntaxErrorException
	{
		...
		if (Calculator.getMode() == Calculator.Mode.REAL)
			return new NumberSubtractExpression(op1, op2);
		throw new SyntaxErrorException("No known operator - for these params.");
	}
}

class NumberSubtractExpression implements MathExpression
{
	MathExpression left, right;

	public NumberSubtractExpression(MathExpression aLeft, MathExpression aRight)
	{
		left = aLeft;
		right = aRight;
	}
