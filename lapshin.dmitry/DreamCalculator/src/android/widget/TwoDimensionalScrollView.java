 ...
 * Date: 02.11.13
 * Time: 19:51
 * Android developers couldn't write this. I will try...
 */
...
public class TwoDimensionalScrollView extends FrameLayout
{
	...
	private static final String TAG = "TwoDimensionalScrollView";
...
//	private StrictMode.Span mFlingStrictSpan = null;
	...
	public TwoDimensionalScrollView(Context context, AttributeSet attrs, int defStyle) {
	 ...
	 */
	/*
		Extremely don't know what to use...
	 */
	...
	*/
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        */
         ...
         */
		if (getScrollY() == 0 && getScrollX() == 0 && !canScrollVertically(1) && !canScrollHorizontally(1)) {
			return false;
		}
		...
		switch (action & MotionEvent.ACTION_MASK) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (mIsBeingDragged) {
					...
					if (overScrollBy(deltaX, deltaY, mScrollX, mScrollY,
							range, range, mOverscrollDistance, mOverscrollDistance, true)) {
				...
				if (mIsBeingDragged) {
					...
					if (getChildCount() > 0) {
						if ((Math.abs(initialVelocity) > mMinimumVelocity)) {
							...
							if (mScroller.springBack(mScrollX, mScrollY, 0, getScrollRange(), 0,
									getScrollRange())) {
								postInvalidateOnAnimation();
							}
						}
			...
			case MotionEvent.ACTION_CANCEL:
				if (mIsBeingDragged && getChildCount() > 0) {
					if (mScroller.springBack(mScrollX, mScrollY, 0, getScrollRange(), 0, getScrollRange())) {
						postInvalidateOnAnimation();
					}
					mActivePointerId = INVALID_POINTER;
					mIsBeingDragged = false;
